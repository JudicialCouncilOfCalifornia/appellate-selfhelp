...
if ( !class_exists( 'avia_sc_testimonial_custom' ) )
{
	class avia_sc_testimonial_custom extends aviaShortcodeTemplate
	{
			static $columnClass;
			static $rows;
			static $counter;
			...
			function shortcode_insert_button()
			{
				...
				$this->config['name']		= __('Custom Testimonials', 'avia_framework' );
				$this->config['tab']		= __('Content Elements', 'avia_framework' );
				$this->config['icon']		= AviaBuilder::$path['imagesURL']."sc-testimonials.png";
				$this->config['order']		= 20;
				$this->config['target']		= 'avia-target-insert';
				$this->config['shortcode'] 	= 'av_custom_testimonials';
				$this->config['shortcode_nested'] = array('av_testimonial_single');
				$this->config['tooltip'] 	= __('Creates a Custom Testimonial Grid', 'avia_framework' );
