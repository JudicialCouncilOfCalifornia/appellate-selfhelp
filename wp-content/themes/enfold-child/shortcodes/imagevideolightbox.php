...
if ( !class_exists( 'avia_sc_image_videolightbox' ) )
{
	class avia_sc_image_videolightbox extends aviaShortcodeTemplate
	{
			/**
			...
			function shortcode_insert_button()
			{
				...
				$this->config['name']			= __('Image Video Lightbox', 'avia_framework' );
				$this->config['tab']			= __('Media Elements', 'avia_framework' );
				$this->config['icon']			= AviaBuilder::$path['imagesURL']."sc-image.png";
				$this->config['order']			= 100;
				$this->config['target']			= 'avia-target-insert';
				$this->config['shortcode'] 		= 'av_image_videolightbox';
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				if(!empty($attachment))
				{
					...
					if(!empty($attachment_entry))
					{
						...
						$linktourl = get_post_meta( $attachment_entry->ID, '_carousel_slider_link_url', true );						
				...
				if(!empty($src))
				{
					...
					else
					{
                        ...
                        $output .= "<div class='img-videolightbox avia-image-container {$class} {$av_display_classes} ".$meta['el_class']." ".$this->class_by_arguments('align' ,$atts, true)."' $markup >";
                        $output .= "<div class='avia-image-container-inner'>";
						if($link)
						{
							$output.= "<a href='{$linktourl}' class='avia_image video-img-icon' {$blank}><span class='overlay-icon'><span class='icon-overlay-inside'></span></span><img class='avia_image ' src='{$src}' alt='{$alt}' title='{$title}' $markup_url /></a>{$overlay} ";
						}
						else
						{
							$output.= "<img class='avia_image ' src='{$src}' alt='{$alt}' title='{$title}'  $markup_url />{$overlay}";
						}
                        $output .= "</div>";
                        $output .= "</div>";
					}
