<?php
/**
 * Resources List Shortcode
 * 
 * Creates a list with nice icons beside
 */
if ( ! defined( 'ABSPATH' ) ) {  exit;  }    // Exit if accessed directly

if ( !class_exists( 'avia_sc_resourcelist' ) )
{
	class avia_sc_resourcelist extends aviaShortcodeTemplate
	{
			/**
			...
			function shortcode_insert_button()
			{
				...
				$this->config['name']		= __('Resources List', 'avia_framework' );
				$this->config['tab']		= __('Content Elements', 'avia_framework' );
				$this->config['icon']		= AviaBuilder::$path['imagesURL']."sc-iconlist.png";
				$this->config['order']		= 90;
				$this->config['target']		= 'avia-target-insert';
				$this->config['shortcode'] 	= 'av_resourceslist';
				$this->config['shortcode_nested'] = array('av_resourceslist_item');
			...
			function popup_elements()
			{
				$this->elements = array(
					...
					array(
						...
						array(
									"name" 	=> __("Resources Title", 'avia_framework' ),
									"desc" 	=> __("Enter the Resources Title", 'avia_framework' ) ,
									"id" 	=> "resource_title",
									"std" 	=> "Resources Title",
									"type" 	=> "input"),
						array(
						"name" 	=> __("Download Link", 'avia_framework' ),
						"desc" 	=> __("Change resources link to download", 'avia_framework' ),
						"id" 	=> "resource_download",
						"type" 	=> "select",
						"std" 	=> "",
						"subtype" => array(	__('No', 'avia_framework' )  =>'no',
						...
						array(
							...
							'subelements' 	=> array(
								...
								array(
									"name" 	=> __("Browse all link?", 'avia_framework' ),
									"desc" 	=> __("Do you want to add browse all link?", 'avia_framework' ),
									"id" 	=> "browsealllink",
									"type" 	=> "select",
									"std" 	=> "",
									"subtype" => array(	__('No', 'avia_framework' )  =>'no',
					...
					array(
						...
						"type" 	=> "select",
						"std" 	=> "",
						"subtype" => array(	__('Default (Minimal small list)', 'avia_framework' )  =>'av-iconlist-small',
											__('Big List', 'avia_framework' ) =>'',
					)),
					...
					array(	
						"name" 	=> __("section class", 'avia_framework' ),
						"desc" 	=> __("Enter a custom class", 'avia_framework' ),
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				if($custom_section_class)
				{					
					$this->resource_class = $custom_section_class;					
				}				
				if($resource_title)
				{					
				...
				{					
					$this->resource_download = $resource_download == 'yes' ? "download" : "";					
				}								
				...
				$output	 = "";
				$output .= "<div class='avia-resource-list-container avia-icon-list-container {$av_display_classes} {$this->resource_class} ".$meta['el_class']."'>";
				$output .="<h3>{$this->resource_title}</h3>";
			...
			function av_resourceslist_item($atts, $content = "", $shortcodename = "")
			{
				
				extract($this->screen_options); //return $av_font_classes, $av_title_font_classes and $av_display_classes
				
                $atts =  shortcode_atts(array('title' => '', 'link' => '', 'browsealllink' => '', 'icon' => '', 'font' =>'', 'linkelement' =>'', 'linktarget' => '', 'custom_markup' => '', 
                
                ), $atts, 'av_resourceslist_item');
                        ...
                        switch($atts['linkelement'])
                        {
                            case 'both':
								if($atts['browsealllink'] == 'yes'){
									...
									if($atts['title']) $atts['title'] = "<a href='{$atts['link']}' {$this->resource_download} title='".esc_attr($linktitle)."'{$blank}>{$linktitle}</a>";
									$display_char_wrapper['start'] = "a href='{$atts['link']}' {$this->resource_download} title='".esc_attr($linktitle)."' {$blank}";									
								}
				...
				$atts['browsealllink'] == 'yes' ? $custom_icons =  "<div class='browse-all-div'></div>" : $custom_icons = "<{$display_char_wrapper['start']} {$this->icon_html_styling} class='iconlist_icon  avia-font-".$atts['font']."'><span class='iconlist-char ' {$display_char}></span><span class='iconlist-char ' {$display_char}></span></{$display_char_wrapper['end']}>";
				$output  = "";
				$output .= "<li>";
				$output .= 		"{$custom_icons}";
				...
				if(!empty($atts['title'])) $output .="<{$title_el} class='av_resourceslist_title iconlist_title{$iconlist_title} {$this->title_class} {$av_title_font_classes}' {$markup} {$this->title_styling}>".$atts['title']."</{$title_el}>";
                $output .=                  '</header>';
