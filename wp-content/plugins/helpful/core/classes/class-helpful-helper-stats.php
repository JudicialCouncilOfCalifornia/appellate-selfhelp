...
class Helpful_Helper_Stats {
	...
	public static function getContra( $post_id = null, $percentages = false ) {
		...
		$pro            = (int) self::getPro( $post_id );
		$contra_percent = 0;

		if ( 0 !== $contra ) {
			$contra_percent = (int) ( ( $contra / ( $pro + $contra ) ) * 100 );
		}
	...
	public static function getProAll( $percentages = false ) {
		...
		$pro         = ( $var ? (int) $var : 0 );
		$contra      = (int) self::getContraAll();
		$pro_percent = 0;

		if ( 0 !== $pro ) {
			$pro_percent = (int) ( ( $pro / ( $pro + $contra ) ) * 100 );
		}
