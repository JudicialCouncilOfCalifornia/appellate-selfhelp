    ...
    <div <?php echo join( " ", $this->carousel_options( $id ) ); ?>>
		...
		foreach ( $images_ids as $image_id ):
			...
			if ( $_show_lightbox == 'on' && filter_var( $image_link_url, FILTER_VALIDATE_URL )) {
				$image_src = wp_get_attachment_image_src( $image_id, 'full' );								
				echo sprintf( '<a href="%1$s" class="magnific-popup video-img-icon"><span class="overlay-icon"><span class="icon-overlay-inside"></span></span>%2$s%3$s</a>', esc_url( $image_link_url ), $image, $full_caption, $id );
