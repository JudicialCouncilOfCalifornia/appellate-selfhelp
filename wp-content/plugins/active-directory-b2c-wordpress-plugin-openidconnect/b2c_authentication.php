...
function b2c_login() {
	...
	if (strpos($uri, 'redirect_to') === false && (!isset($_COOKIE['wp_admin']) || $_COOKIE['wp_admin'] != 1)) {	
		try {
			unset($_COOKIE['wp_admin']);
			setcookie('wp_admin','', time() - 3600);			
		...
		setcookie('wp_admin', true, time() + 3600);
	}
}

/**
 * Redirects to B2C on a user password reset.
 */
function b2c_password_reset() {
	if($_GET['action'] =="lostpassword" && $_GET['source'] == 'openid'){
		try {						
			$b2c_endpoint_handler = new B2C_Endpoint_Handler(B2C_Settings::$forgot_password_policy);
			$authorization_endpoint = $b2c_endpoint_handler->get_authorization_endpoint()."&state=forgot_password";
			ob_start();			
			wp_redirect($authorization_endpoint);							
		}
		catch (Exception $e) {
...
function b2c_verify_token() {
	try {
		...
		if (isset($_POST['error'])) {			
			...
			echo '<br/>error:' . $_POST['error_description'];	*/	
			if(preg_match("/AADB2C90118/", $_POST['error_description'])){
				wp_redirect($signout_uri."/wp-login.php?action=lostpassword&source=openid");
			}else{				
				//wp_redirect($signout_uri."/");				
		...
		if (isset($_POST[B2C_RESPONSE_MODE])) {			
			...
			$user = WP_User::get_data_by('login', $b2cID);
			$global_user = false;				
			...
			if ( current_user_can( 'manage_options' ) ) {
				...
				wp_safe_redirect(site_url() . '/prepare-doc');
			}
			exit;			
		}
	} catch (Exception $e) {
 ...
 */
add_action('wp_loaded', 'b2c_password_reset');
