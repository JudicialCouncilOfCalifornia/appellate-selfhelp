...
class B2C_Settings_Page
{
    ...
    {        
		...
		add_settings_field(
            'b2c_forgot_password_policy_id', // ID
            'Edit Forgot Password Policy', // Title 
            array( $this, 'b2c_forgot_password_policy_id_callback' ), // Callback
            'b2c-settings-page', // Page
            'service_config_section' // Section           
        );
    ...
    public function sanitize( $input )
    {
		...
		if( isset( $input['b2c_forgot_password_policy_id'] ) )
            $new_input['b2c_forgot_password_policy_id'] = sanitize_text_field(strtolower( $input['b2c_forgot_password_policy_id'] ));
     ...
     * Get the settings option array and print one of its values
     */
    public function b2c_forgot_password_policy_id_callback()
    {
        printf(
            '<input type="text" id="b2c_forgot_password_policy_id" name="b2c_config_elements[b2c_forgot_password_policy_id]" value="%s" />',
            isset( $this->options['b2c_forgot_password_policy_id'] ) ? esc_attr( $this->options['b2c_forgot_password_policy_id']) : ''
        );
    }
